...
RSpec.describe Mutant::CLI::Command::Session do
  ...
  let(:list_header) do
    '%-36s  %-20s  %-10s  %-10s  %s' % ['SESSION ID', 'TIMESTAMP', 'VERSION', 'RUBY', 'SUBJECTS']
  end

  let(:list_row) do
    '%-36s  %-20s  %-10s  %-10s  %s' % [session_id, expected_timestamp, '1.0.0', '4.0.1', 0]
  end
  ...
  describe 'list' do
    ...
    context 'when results directory has an unparseable session' do
      ...
      let(:unsupported_row) do
        '%-36s  %-20s  %-10s  %-10s  %s' % ['bad-file', nil, Unparser::Color::RED.format('[unsupported]'), nil, nil]
      end
  ...
  describe 'show' do
    ...
    context 'with explicit session ID and valid file' do
      ...
      it 'prints session metadata' do
        verify_events { expect(apply(['show', session_id])).to be(true) }
      end
    end
    ...
    context 'with explicit session ID and alive mutations' do
      ...
      let(:session_with_alive_json) do
        <<~'JSON'
          {"mutant_version":"1.0.0","pid":12345,"ruby_version":"4.0.1","session_id":"019cf6f1-77e8-74b6-82db-f8b5faf570cd","subject_results":[{"amount_mutations":1,"coverage_results":[{"mutation_result":{"isolation_result":{"exception":null,"log":"","process_status":null,"timeout":null,"value":{"job_index":0,"passed":false,"runtime":0.1,"output":""}},"mutation_diff":"@@ -1 +1 @@\n-true\n+nil\n","mutation_identification":"evil:Baz#qux:baz.rb:1:ghi56","mutation_node":"nil","mutation_source":"nil","mutation_type":"evil","runtime":0.1},"criteria_result":{"process_abort":false,"test_result":true,"timeout":false}}],"identification":"Baz#qux:baz.rb:1","node":"true","source":"true","source_path":"baz.rb","tests":["test-b"]},{"amount_mutations":2,"coverage_results":[{"mutation_result":{"isolation_result":{"exception":null,"log":"","process_status":null,"timeout":null,"value":{"job_index":0,"passed":true,"runtime":0.1,"output":""}},"mutation_diff":"@@ -1 +1 @@\n-true\n+false\n","mutation_identification":"evil:Foo#bar:foo.rb:1:abc12","mutation_node":"false","mutation_source":"false","mutation_type":"evil","runtime":0.1},"criteria_result":{"process_abort":false,"test_result":false,"timeout":false}},{"mutation_result":{"isolation_result":{"exception":null,"log":"","process_status":null,"timeout":null,"value":{"job_index":0,"passed":true,"runtime":0.1,"output":""}},"mutation_diff":"@@ -1 +1 @@\n-true\n+nil\n","mutation_identification":"evil:Foo#bar:foo.rb:1:def34","mutation_node":"false","mutation_source":"false","mutation_type":"evil","runtime":0.1},"criteria_result":{"process_abort":false,"test_result":false,"timeout":false}}],"identification":"Foo#bar:foo.rb:1","node":"true","source":"true","source_path":"foo.rb","tests":["test-a"]}]}
        JSON
      end
      ...
      let(:raw_expectations) do
        [
          ...
          puts_stdout('Alive:    2'),
          puts_stdout(Mutant::Reporter::CLI::Printer::AliveResults::ALIVE_EXPLANATION),
    ...
    context 'with explicit session ID and missing file' do
      ...
      it 'returns false' do
        verify_events { expect(apply(['show', session_id])).to be(false) }
      end
    end
    ...
    context 'with explicit session ID and invalid JSON' do
      let(:raw_expectations) do
        [
          ...
          puts_stderr("Failed to load session: unexpected token 'not' at line 1 column 1")
        ]
      end
    ...
    context 'with too many arguments' do
      it 'returns error with exact message' do
        expect(parse(%w[show a b])).to eql(
          Mutant::Either::Left.new('Expected zero or one session ID argument')
        )
      end
    end
  ...
  describe 'subjects' do
    ...
    # Bar has 1 alive, sorted first
    let(:bar_row)         { '%-6s  %-6s  %s' % [1, 1, 'Bar#baz:bar.rb:1'] }
    let(:foo_row)         { '%-6s  %-6s  %s' % [0, 0, 'Foo#bar:foo.rb:1'] }
    ...
    it 'prints subjects sorted by alive count descending' do
      verify_events { expect(apply(%w[subjects])).to be(true) }
    end
  end
