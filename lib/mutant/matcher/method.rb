module Mutant
  class Matcher
    ...
    class Method < self
      ...
      SOURCE_LOCATION_WARNING =
        '%s does not have valid source location, unable to emit subject'.freeze
      ...
      private
        ...
        def skip?
          ...
          if location.nil? || BLACKLIST.match(location.first)
            env.warn(SOURCE_LOCATION_WARNING % target_method)
          elsif matched_node_path.any?(&method(:n_block?))
            env.warn(CLOSURE_WARNING % target_method)
          end
        end
