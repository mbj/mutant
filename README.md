...
- Only for the EU: A valid VAT-ID is _required_, no sales to private customers to avoid the
  horrors cross border VAT / MOSS.
  VAT for customers outside of Malta will use **reverse charging**.
- _Per developer_ the git author email address as returned by `git config user.email`
